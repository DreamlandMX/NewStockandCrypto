...
CREATE POLICY "Users can create versions of their notes" ON public.note_versions
...
CREATE POLICY "Users can delete their DMs" ON public.direct_messages
...
CREATE POLICY "Presence is viewable by authenticated users" ON public.user_presence
    FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can update their presence" ON public.user_presence
    FOR ALL USING (auth.uid() = user_id);

-- 2.5 User Profiles Enhancement
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS display_name TEXT,
ADD COLUMN IF NOT EXISTS timezone TEXT DEFAULT 'UTC',
ADD COLUMN IF NOT EXISTS notification_preferences JSONB DEFAULT '{"mentions": true, "dm": true, "channel": false}'::jsonb;

-- 2.6 Custom Channels Table
...
CREATE POLICY "Public channels are viewable" ON public.custom_channels
    FOR SELECT USING (is_public = true OR EXISTS (
        SELECT 1 FROM public.custom_channel_members 
        WHERE custom_channel_members.channel_id = custom_channels.id 
        AND custom_channel_members.user_id = auth.uid()
    ));

CREATE POLICY "Users can create channels" ON public.custom_channels
    FOR INSERT WITH CHECK (auth.uid() = created_by);

CREATE POLICY "Channel members view" ON public.custom_channel_members
    FOR SELECT USING (auth.uid() IS NOT NULL);

CREATE POLICY "Users can join channels" ON public.custom_channel_members
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- 2.7 Typing Indicator Table (ephemeral)
...
CREATE TRIGGER create_presence_trigger
...
ALTER PUBLICATION supabase_realtime ADD TABLE IF EXISTS public.note_versions;
ALTER PUBLICATION supabase_realtime ADD TABLE IF EXISTS public.message_reactions;
ALTER PUBLICATION supabase_realtime ADD TABLE IF EXISTS public.direct_messages;
ALTER PUBLICATION supabase_realtime ADD TABLE IF EXISTS public.user_presence;
ALTER PUBLICATION supabase_realtime ADD TABLE IF EXISTS public.typing_indicators;
...
SELECT 
    ...
    CASE WHEN dm.sender_id = auth.uid() THEN true ELSE dm.is_read END as is_read
FROM latest_messages lm
JOIN public.profiles p1 ON p1.id = lm.user1
JOIN public.profiles p2 ON p2.id = lm.user2
JOIN public.direct_messages dm ON 
    ((dm.sender_id = lm.user1 AND dm.receiver_id = lm.user2) OR 
     (dm.sender_id = lm.user2 AND dm.receiver_id = lm.user1))
    AND dm.created_at = lm.last_message_at
WHERE auth.uid() IN (lm.user1, lm.user2);

-- ==================== COMPLETION ====================
DO $$
BEGIN
    RAISE NOTICE 'Notes and chat enhancement migrations completed.';
    RAISE NOTICE 'New tables: note_versions, message_reactions, direct_messages, user_presence, custom_channels, typing_indicators';
    RAISE NOTICE 'Columns added to: notes, chat_messages, profiles';
    RAISE NOTICE 'Realtime enabled for all new tables';
END $$;
